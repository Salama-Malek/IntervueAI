...
// Lightweight question seeds per role
const SEED_QUESTIONS: Record<Role, string[]> = {
  Frontend: [
...
// Simple scoring similar to frontend heuristic
...
const ROLE_KEYWORDS: Record<Role, string[]> = {
  ...
  'Product Manager': ['metric', 'OKR', 'A/B', 'cohort', 'MVP', 'roadmap', 'user', 'retention', 'activation', 'MVP'],
...
function scoreLengthNormalized(w: number) {
...
function scoreTechnical(t: string, role: Role) {
  const L = t.toLowerCase();
  const found = ROLE_KEYWORDS[role].filter((k) => L.includes(k.toLowerCase()));
  return Math.min(1, found.length / 10);
}
function generateFeedback(length: number, structure: number, technical: number, wordCount: number) {
  const fb: string[] = [];
  if (length < 0.7) {
...
app.post('/sessions/start', (req, res) => {
  const body: InterviewConfig = req.body ?? {};
...
app.post('/sessions/:id/message', (req, res) => {
  const session = sessions.get(req.params.id);
  if (!session) return res.status(404).json({ message: 'Session not found' });
  const { userText } = (req.body ?? {}) as { userText?: string };
